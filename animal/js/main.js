...
/*  1. 클릭한 li에서 data-content 값을 가져와서
            ==> tap_item 중에 해당 값이 id인 요소를 찾아서 나타나게 해야함
    2. 클릭한 li에만 active 클래스 부여 
    3. 클릭한 li안에 있는 span에 선택됨이라고 글자 써줌(다른 li에 있는 건 삭제)
    4. 클릭한 li속성 aria-selected 값을 true로 변경 (다른 li는 모두 false) */
    ...
    $('.find .list .tap_list ul li').on('click', function(){
        ...
        if($(this).hasClass('active') == false){
            ...
            $('.find .list .tap_content .tap_item').removeClass('active')
            $('.find .list .tap_content').find('#'+find_content).addClass('active')
